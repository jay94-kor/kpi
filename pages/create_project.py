...
with st.form("project_creation_form"):
    ...
    role_count = st.number_input('역할 수', min_value=1, max_value=10, value=2, step=1, key='role_count')
    ...
    for i in range(role_count):
        with st.expander(f"역할 {i + 1}", expanded=True):
            ...
            task_count = st.number_input(f'테스크 수 (역할 {i + 1})', min_value=1, max_value=10, value=1, step=1, key=f'task_count_{i}')
    ...
    if submitted:
        ...
        elif any(not role['tasks'] for role in roles):
